...
var Registration = require('./api/registration/registration.model'),
    ...
    RegistrationController = require('./api/registration/registration.controller'),
    ObjectId = require('mongoose').Types.ObjectId;

var agenda = new Agenda({db: { address: config.mongo.uri }});

// Create Agenda Job and Reminder
agenda.define('Send Mail and SMS Reminder', function(job, done){
    // Get the User Data
    Registration.findOne({user: new ObjectId(job.attrs.data.userId), statusConfirmed:true, paymentSuccessful:true}, function(err, registration){
        mailer.sendReminderMail(registration.email, job.attrs.data.session, function(){
            mailer.sendReminderSMS(registration.mobile, job.attrs.data.session, done);
        });
    });
});
...
agenda.define('Update Web Transactions For Individuals', function(job, done) {
    ...
    Registration.find({completed: true, webpay: true, lastModified: { $gt: cutoff }}, function(err, toResolve){
...
agenda.define('Update Web Transactions For Groups', function(job, done) {
    var cutoff = moment().subtract(2,'h');
    Invoice.find({finalized: true, webpay: true, lastModified: { $gt: cutoff }}, function(err, toResolve){
...
agenda.every('0.11 hours', [ 'Delete Incomplete Invoices', 'delete old registrations' ]);

agenda.every('2.71 hours', [ 'Create Accounts for Paid Invoices' ]);

agenda.every('5 minutes', [ 'Create Accounts for Direct Bank Registrations' ]);

agenda.every('17 minutes', [ 'Update Web Transactions For Individuals' ]);

agenda.every('29 minutes', [ 'Send Web Payment Success Email for Individuals', 'Send Web Payment Success SMS for Individuals', 'Send Direct Registration Success SMS for Individuals']);

agenda.every('37 minutes', [ 'Send Direct Registration Success Email for Individuals', 'Update Web Transactions For Groups' ]);

agenda.every('23 minutes', [ 'Send Bank Payment Success SMS for Individuals', 'Send Bank Payment Success Email for Individuals' ]);

agenda.every('.02 hours', [ 'Send Bank Payment Success SMS for Groups', 'Send Bank Payment Success Email for Groups', 'Send Web Payment Success SMS for Groups', 'Send Web Payment Success Email for Groups' ]);
